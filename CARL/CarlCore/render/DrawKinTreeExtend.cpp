...
void cDrawKinTreeExtend::Draw(const Eigen::MatrixXd& joint_desc, const Eigen::VectorXd& pose, double link_width, const tVector& fill_col, const tVector& line_col)
{
...
void cDrawKinTreeExtend::DrawTree(const Eigen::MatrixXd& joint_desc, const Eigen::VectorXd& pose, int joint_id, double link_width,
	const tVector& fill_col, const tVector& line_col)
{
	...
	if (joint_id != cKinTree::gInvalidJointID)
	{
		...
		if (has_parent)
		{
			...
			float bone_width = 12.5f * link_width * len;
			tVector size = tVector(bone_width, len, bone_width, 0);
			...
			cDrawUtilExtend::DrawBone(tVector::Zero(), size, cDrawUtil::eDrawSolid);

			if (line_col[3] > 0)
			{
				cDrawUtil::SetColor(tVector(line_col[0] * 1.2, line_col[1] * 1.2, line_col[2] * 1.2, line_col[3]));
			}
		...
		cDrawUtil::DrawSphere(node_radius * 0.5f);
